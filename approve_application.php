...
if (isset($_POST['application_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $update_stmt = $conn->prepare("UPDATE project_applications SET status = 'approved' WHERE id = ?");
        $update_stmt->bind_param("i", $application_id);
        $update_stmt->execute();

        $notification_message = "Your application for project {$project_title} has been approved.";
        ...
        echo "<script>alert('Application approved successfully.'); window.location.href = 'view_project.php?project_id={$application['project_id']}';</script>";
    } else {
